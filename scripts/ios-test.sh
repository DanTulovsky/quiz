#!/bin/bash
# Run iOS tests with configurable destination
# Defaults to Mac (Designed for iPad) to avoid simulator clone issues
# Usage: ios-test.sh [-p] [DESTINATION=...]
...
if [ "$PICK_MODE" = true ]; then
  echo "📋 Fetching available test destinations..."
  ...
  # Default to Mac (Designed for iPad) to avoid simulator clone issues
  # Use the id from available destinations (more reliable than variant)
  MAC_DEST=$(xcodebuild -scheme Quiz -showdestinations 2>/dev/null | \
    grep -E "^\s+\{.*platform:macOS" | head -1 | \
    awk '{
      line = $0
      id = ""
      if (match(line, /id:([^,}]+)/)) {
  ...
  echo "💻 Using Mac (Designed for iPad) - no simulator clones will be created"
fi
