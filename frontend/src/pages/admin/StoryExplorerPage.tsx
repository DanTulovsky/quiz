...
const StoryExplorerPage: React.FC = () => {
  ...
  const userOptions = useMemo(() => {
    ...
    return users
      .filter((u: any) => u?.id)
      .map((u: any) => ({ value: String(u.id), label: u.username || `user-${u.id}` }));
  }, [usersData]);
  ...
  return (
    <Container size='xl' py='md'>
      ...
      <Text c='dimmed' mb='xl'>
        Browse users' stories. Use filters to narrow results and click a row to view the full story.
      </Text>
      ...
      <Card shadow='sm' padding='md' radius='md' withBorder mb='md'>
        <Group justify='space-between' mb='md'>
          ...
          <Button variant='light' size='sm' leftSection={<IconFilter size={16} />} onClick={clearFilters}>
            Clear All Filters
          </Button>
        </Group>
        <Group gap='sm' align='center' style={{ flexWrap: 'wrap' }}>
          ...
          <Select
            ...
            data={[
              ...
              ...(languagesData?.map(lang => ({ value: lang, label: lang.charAt(0).toUpperCase() + lang.slice(1) })) || []),
            ]}
            clearable
            size='xs'
            style={{ minWidth: 160 }}
      ...
      <Card withBorder>
        ...
        <div style={{ maxHeight: '600px', overflowY: 'auto' }}>
          <Table>
            ...
            <Table.Tbody>
              {stories.map((s: any) => (
                <Table.Tr key={s.id} onClick={() => openView(Number(s.id))} style={{ cursor: 'pointer' }}>
                  ...
                  <Table.Td>
                    <Badge variant='light' color={s.status === 'active' ? 'green' : s.status === 'completed' ? 'blue' : 'gray'}>
                      {s.status}
                    </Badge>
                  </Table.Td>
                  <Table.Td>{s.user_id}</Table.Td>
                  <Table.Td>
                    <Button size='xs' variant='subtle' onClick={e => { e.stopPropagation(); openView(Number(s.id)); }}>
                      View
                    </Button>
                  </Table.Td>
                </Table.Tr>
        ...
        <Group justify='space-between' p='md'>
          ...
          {totalStories > 0 && (
            <Pagination total={Math.ceil(totalStories / pageSize)} value={currentPage} onChange={setCurrentPage} size='sm' />
          )}
        </Group>
      </Card>

      {/* View Modal */}
      <Modal opened={viewModalOpen} onClose={() => setViewModalOpen(false)} title='View Story' size='xl'>
        <Stack>
          {!selectedStory ? (
                ...
                <Stack gap='sm'>
                  <Group>
                    <Select
                      ...
                      data={(selectedStory.sections || []).map((sec, idx) => ({ value: String(idx), label: `Section ${sec.section_number}` }))}
                      style={{ minWidth: 200 }}
                    />
                  </Group>
                  <StorySectionView
                    section={(selectedStory.sections || [])[sectionIndex] || null}
                    ...
                    onNext={() => setSectionIndex(i => Math.min((selectedStory.sections?.length || 1) - 1, i + 1))}
                    onFirst={() => setSectionIndex(0)}
                    onLast={() => setSectionIndex(Math.max(0, (selectedStory.sections?.length || 1) - 1))}
                  />
                </Stack>
              </Tabs.Panel>
            </Tabs>
