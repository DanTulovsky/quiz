...
export async function ensureLanguagesLoaded(): Promise<void> {
    ...
    runtimeLoadingPromise = (async () => {
        try {
            ...
            type LanguagesResponse = {languages?: Array<{code: string; name: string}>};
            const data: LanguagesResponse | Array<{code: string; name: string}> =
                await res.json();

            const list: Array<{code: string; name: string}> = Array.isArray(data)
                ? data
                : Array.isArray((data as LanguagesResponse).languages)
                    ? ((data as LanguagesResponse).languages as Array<{
                        code: string;
                        name: string;
                    }>)
                    : [];
