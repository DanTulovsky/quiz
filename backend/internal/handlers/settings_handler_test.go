...
func setupSettingsTestRouter(learningService services.LearningServiceInterface) *gin.Engine {
	...
	handler := NewSettingsHandler(nil, nil, learningService, mockEmailService, mockConfig, observability.NewLogger(&config.OpenTelemetryConfig{EnableLogging: false}))
...
func TestGetProviders(t *testing.T) {
	...
	mockEmailService := &MockEmailService{}
	handler := NewSettingsHandler(nil, nil, nil, mockEmailService, mockConfig, observability.NewLogger(&config.OpenTelemetryConfig{EnableLogging: false}))
