...
func setupSettingsTestRouter(learningService services.LearningServiceInterface) *gin.Engine {
	...
	handler := NewSettingsHandler(nil, nil, nil, nil, nil, learningService, mockEmailService, nil, mockConfig, observability.NewLogger(&config.OpenTelemetryConfig{EnableLogging: false}))
...
func TestGetProviders(t *testing.T) {
	...
	mockEmailService := &MockEmailService{}
	handler := NewSettingsHandler(nil, nil, nil, nil, nil, nil, mockEmailService, nil, mockConfig, observability.NewLogger(&config.OpenTelemetryConfig{EnableLogging: false}))
