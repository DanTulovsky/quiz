...
-- Add section_id column (references story_sections.id) if it doesn't exist
...
ADD COLUMN section_id INTEGER REFERENCES story_sections(id) ON DELETE
SET NULL;
END IF;
END $$;
-- Add story_id column (references stories.id) if it doesn't exist
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_name = 'snippets'
...
ALTER TABLE snippets
ADD COLUMN story_id INTEGER REFERENCES stories(id) ON DELETE
SET NULL;
END IF;
END $$;
